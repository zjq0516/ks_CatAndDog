...
#2.预测-先对测试集进行预测，比较模型的预测结果和真实标签，进而评估模型的性能和准确度
...
print("召回率：", recall_score(test_label, result, zero_division=1, average='macro'))
...
for first_index in range(len(confusion)):
    for second_index in range(len(confusion[first_index])):
        ...
        plt.text(first_index, second_index, confusion[first_index][second_index],
                 fontsize=18, color=color, verticalalignment='center', horizontalalignment='center')
plt.show()
